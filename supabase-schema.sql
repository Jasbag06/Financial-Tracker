...
-- ============ BUDGETS ============
-- Scoped to an Event first, then a category within it - trip_id = null means
-- "General" (resets every month); a trip-scoped budget covers the whole event.
...
-- A plain unique(user_id, trip_id, category) wouldn't stop duplicate General
-- budgets for the same category, since Postgres treats every null trip_id as
-- distinct - so General and event-scoped budgets each get their own partial
...
-- ============ DEBTS / IOUs ============
-- Deliberately kept OUT of the transactions table: lending money out isn't
-- really an expense (it's coming back), so counting it in the Monthly Recap
-- would make an ordinary month look like a blowout. Only an actual repayment
-- can optionally write a real transaction (debt_payments.transaction_id).
...
-- generated by the app for the owning user, never a public link. Files are
-- stored as "{user_id}/{transaction_id}.jpg", so the first path segment
-- doubling as the owner check is enough to keep receipts private.
