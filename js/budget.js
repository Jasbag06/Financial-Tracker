...
Catetin.renderBudget = function () {
  document.getElementById('budget-month-label').textContent = new Date().toLocaleDateString('id-ID', { month: 'long', year: 'numeric' });
  ...
  if (cats.length === 0) {
    container.innerHTML = '<div class="empty-state">Belum ada kategori dengan budget.<br>Atur di Pengaturan &rarr; Kelola Kategori.</div>';
    return;
  }
  ...
  container.innerHTML = cats.map(function (c) {
    ...
    return '<div class="budget-row"><div class="between"><span style="font-size:13.5px;font-weight:600;">' + c.icon + ' ' + Catetin.escapeHtml(c.name) + '</span><span class="mono pill" style="background:' + pillBg + ';color:' + pillColor + ';">' + pct + '%</span></div>'
      ...
      + '<div class="between muted mono" style="font-size:11px;margin-top:4px;"><span>' + Catetin.fmtRp(spent) + '</span><span>dari ' + Catetin.fmtRp(c.budget_monthly) + '</span></div></div>';
  }).join('');
};
